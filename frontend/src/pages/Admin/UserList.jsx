...
const UserList = () => {
    ...
    const deleteHandler = async (userId) => {
        if(window.confirm("Sure to proceed?")) {
            try {
                                ...
                                <tr key={user._id}>
                                    ...
                                    <td className="px-4 py-2">
                                        {editableUserId === user._id ? (
                                            ...
                                            <button onClick={() => updateHandler(user._id)} className="ml-2 bg-indigo-700 text-white py-2 px-4 rounded-lg">
                                                <FaCheck />
                                            </button>
                                            </div>               
                                        ) : (
                                    ...
                                    <td className="px-4 py-2">
                                        {editableUserId === user._id ? (
                                            <div className="flex items-center">
                                                <input type="email" value={editableUserEmail} 
                                    ...
                                    <td className="px-4 py-2">
                                        ...
                                        <td className="px-4 py-2">
                                            {!user.isAdmin && (
                                                <div className="flex">
                                                    <button onClick={() => deleteHandler(user._id)} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg">
                                                        <FaTrash />
                                                    </button>
                                                </div>    
                                            )}
